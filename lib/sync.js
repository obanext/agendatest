...
async function fetchPage(branchName, when, page) {
  ...
  const url =
    ...
    `&facet=${wanneerFacet}${branchParam}&page=${page}`;
...
async function ensureCache() {
  ...
  if (needsSync(existing)) return await runFullSync();
  return existing;
}
...
export async function getAgendaForBranch(branchName) {
  ...
  const realName = Object.keys(cache.branches).find(
    b => b.toLowerCase() === wanted
  );
  ...
  if (!realName) {
    return {
      ...
      pages: 0,
      items: [],
      counts: { b_thisweek: 0, c_nextweek: 0 },
      error: "Branch not found"
    };
  }

  const branch = cache.branches[realName];
